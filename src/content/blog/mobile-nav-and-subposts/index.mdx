...
This site is based from <a href='https://astro-erudite.vercel.app/'>astro-erudite's v1.6.0</a> release, which I made some modifications to fit my personal taste and preferences. I mostly kept the original features and design, but tweaked a few things here and there. I am mostly dedicated to give better <abbr title="User Experience">UX</abbr> in mobile view while keeping the desktop view and styles intact. 
...
### The Logic
The script only runs a function that initially checks if the user is on the landing or homepage, which is where the grid style is. 
...
Then, when the user scrolls down past a certain threshold (100 pixels in this case), the header becomes visible by removing the `-translate-y-full` class, which is responsible for hiding it off-screen. When the user scrolls back up to the top of the page, the header hides again by adding the same class back.
